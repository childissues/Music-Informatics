...
if __name__ == "__main__":
    ...
    if args.submission is None:
        default_submission = "results_key_estimation.csv"
        if os.path.exists(default_submission):
            print(f"No submission file provided. Using default: {default_submission}")
            args.submission = default_submission
        else:
            print("Usage: python key_estimation_challenge.py --submission <submission_file.csv> --target <ground_truth.csv>")
            print("Note: This script evaluates a CSV submission file.")
            print("To run the main Key Estimation wrapper, use: python J_S_Bach_KeyEstimation.py -i <data_dir>")
            exit(0)

    if args.target is None:
        # Try to find the training GT file
        default_target = os.path.join("key_estimation_dataset", "train", "key_estimation_train_gt.csv")
        if os.path.exists(default_target):
             print(f"No target file provided. Using default ground truth: {default_target}")
